# API Reporte DTGP Cartagena - Integración con Bot Maria

## Endpoint

```
GET https://metasuite.dtgrowthpartners.com/api/report/dtgp?accessToken={TOKEN}
```

## Autenticación

El endpoint requiere un **Meta Access Token** válido con acceso a la cuenta publicitaria `act_781485172384812` (DTGP Cartagena).
...
url = f"https://metasuite.dtgrowthpartners.com/api/report/dtgp?accessToken={TOKEN}"
...
7. Costo por mensaje por campaña = spend / contactos_de_mensajes_totales
8. **Costo promedio x msg del RESUMEN** = SOLO sumar gasto de campañas con mensajes > 0 / total mensajes. NO incluir gasto de ThruPlay/reconocimiento/tráfico sin mensajes
...
- El `slug` (`dtgp`) identifica la cuenta.
- La vista web del reporte está disponible en: `https://metasuite.dtgrowthpartners.com/dtgp`
