# API Reporte Importaciones CTG - Integración con Bot Maria

## Endpoint

```
GET https://metasuite.dtgrowthpartners.com/api/report/importaciones?accessToken={TOKEN}
```

## Autenticación

El endpoint requiere un **Meta Access Token** válido con acceso a la cuenta publicitaria de Importaciones CTG.
...
```
Reporte detallado de ayer *{día} de {mes}* — Importaciones CTG 📊
...
- El `slug` (`importaciones`) identifica la cuenta.
- La vista web del reporte está disponible en: `https://metasuite.dtgrowthpartners.com/importaciones`
