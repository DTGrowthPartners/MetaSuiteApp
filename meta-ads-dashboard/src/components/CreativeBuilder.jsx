...
// ============================================
function TemplateSelector({ onSelectTemplate, lang }) {
  ...
  return (
    <div className="template-selector">
      <h2>{tr('select_template', lang)}</h2>
      <p className="subtitle">{tr('select_template_desc', lang)}</p>
      ...
      <div className="category-filter">
        {categories.map(cat => (
          ...
          >
            {cat === 'all' ? tr('all_categories', lang) : tr(cat, lang) || cat}
          </button>
        ))}
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="template-content">
                <h3>{tr('tpl_' + template.id, lang) || template.name}</h3>
                <span className="template-category">{tr(template.category, lang) || template.category}</span>
                <p className="template-description">{tr('tpl_' + template.id + '_desc', lang) || template.description}</p>
                ...
                <div className="template-meta">
                  <span className="meta-item">
                    <strong>{tr('tpl_budget', lang)}:</strong> ${new Intl.NumberFormat('es-CO').format(suggestedBudget)} {tr('tpl_per_day', lang)}
                  </span>
                  <span className="meta-item">
                    <strong>CTAs:</strong> {[...new Set(ctaList)].slice(0, 3).map(c => tr(getCTALabel(c), lang) || getCTALabel(c)).join(', ')}
                  </span>
                </div>
              </div>
...
// ============================================
function UploadStep({ adAccounts, onJobCreated, selectedTemplate, onBackToTemplates, accessToken, lang }) {
  ...
  return (
    <div className="upload-step">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          <label>{tr('page_label', lang)} *</label>
          <CustomSelect
            value={selectedPage}
        ...
        <div className="form-group">
          <label>{tr('ig_account', lang)}</label>
          <CustomSelect
        ...
        {budgetLevel === 'campaign' && (
          <div className="form-group">
            <label>{tr('budget_label', lang)} (COP) *</label>
            <input
              type="number"
              placeholder="20000"
              min="5000"
        ...
        <div className="form-group">
          <label>{adSetMode === 'single' ? tr('audience_shared', lang) : tr('audience_default', lang)}</label>
        ...
        <div className="section-card" id="section-anuncios">
          <h4><span className="section-icon"><Film size={18} /></span> {tr('ads_title', lang)} ({ads.length})</h4>

        {/* AdSet Mode Toggle */}
        <div className="form-group">
          <label>{tr('ad_structure', lang)}</label>
          <div className="toggle-group">
            <button
              type="button"
            ...
            >
              <strong>{tr('standard_ad', lang)}</strong>
              <small>{tr('standard_desc', lang)}</small>
            </button>
            {!isDCBlocked && (
              <button
                type="button"
            ...
            {['OUTCOME_SALES', 'OUTCOME_APP_PROMOTION'].includes(selectedTemplate?.objective) && (
              ...
              >
                <strong>{tr('flexible_ad', lang)}</strong>
                <small>{tr('flexible_desc', lang)}</small>
              </button>
            )}
          </div>
          <p className="hint">
        ...
        <button type="submit" className="submit-button" disabled={uploading || loadingAudiences || ads.some(ad => ad.analyzingMedia)}>
          {uploading ? tr('processing', lang) : ads.some(ad => ad.analyzingMedia) ? `${tr('processing', lang)}...` : tr('continue_create', lang)}
        </button>
      </form>
    </div>
  );
...
// ============================================
function DraftStep({ job, onComplete, onBack, accessToken, lang }) {
        ...
        <div className="success-view">
          ...
          <div className="success-view-header">
            ...
            <h2 className="success-view-title">
              {adWasCreated ? tr('campaign_created', lang) : tr('campaign_adset_created', lang)}
            </h2>
          ...
          <div className="success-view-actions">
            ...
            >
              <Globe size={16} /> {tr('open_ads_manager', lang)}
            </a>
            <button className="success-view-btn-primary" onClick={onComplete}>
              <Plus size={16} /> {tr('create_another', lang)}
            </button>
          </div>
        </div>
      </div>
...
// ============================================
export default function CreativeBuilder({ adAccounts, accessToken, lang }) {
    ...
    <div className="creative-builder">
      <div className="progress-steps">
        <div className={`progress-step ${getStepStatus('templates')}`}>
          ...
          <span className="step-label">{tr('step_template', lang)}</span>
        </div>
        <div className="step-connector"></div>
        <div className={`progress-step ${getStepStatus('config')}`}>
          <span className="step-number">2</span>
          <span className="step-label">{tr('step_configure', lang)}</span>
        </div>
        <div className="step-connector"></div>
        <div className={`progress-step ${getStepStatus('draft')}`}>
          <span className="step-number">3</span>
          <span className="step-label">{tr('step_create', lang)}</span>
        </div>
      </div>

      <div className="builder-content">
        {step === 'templates' && (
          <TemplateSelector onSelectTemplate={handleSelectTemplate} lang={lang} />
        )}
